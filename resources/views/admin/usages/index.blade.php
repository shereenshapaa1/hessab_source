...
@section('tab_name', __('admin.Usages'))
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Usages') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('usages.create'))
                            <a class="btn btn-success pull-right text-white" href="{{ route('admin.usages.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                    <hr>
                    @include('admin.usages.search')
                                            ...
                                            <td class="text-left">
                                                @if (can('usages.edit'))
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.usages.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('usages.destroy'))
                                                    <form action="{{ route('admin.usages.destroy', $item->id) }}"
