...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Rates') /
                        @if ($item->id)
                    ...
                    <hr>
                    @if ($item->id)
                        <form class="needs-validation" action="{{ route('admin.rates.update', $item->id) }}" method="POST"
                            enctype='multipart/form-data' novalidate>
                            <input type="hidden" name="_method" value="PUT">
                        @else
                            <form class="needs-validation" action="{{ route('admin.rates.store') }}" method="POST"
                                enctype='multipart/form-data' novalidate>
                    @endif
                    @csrf
