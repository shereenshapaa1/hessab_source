@extends('admin.layouts.app')
@section('tab_name', __('admin.Counters'))

@section('content')
<div class="row">
  <div class="col-12">
    <div class="card ">
      <div class="card-body">
          <h4>
              @lang('admin.Counters') #{{ $item->id }} {!! $item->active_span  !!}
          </h4>
          <hr>
            <table class="table">
                <tbody>

                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i> @lang('admin.Title') En </th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->translate('ar')->name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.Title') Ar </th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->translate('en')->name ?? '' }}
                        </td>
                    </tr>

                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.Position')</th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->position }}
                        </td>
                    </tr>

                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.Counter')</th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->counter }}
                        </td>
                    </tr>
                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.CreationDate') </th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->created_at }}
                        </td>
                    </tr>
                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.LastUpdate')</th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->updated_at }}
                        </td>
                    </tr>

                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.Description') En </th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->translate('ar')->description ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th width="25%"> <i class="icon-minus-box"></i>@lang('admin.Description') Ar </th>
                        <td> <i class="icon-pin"></i>
                            {{ $item->translate('en')->description ?? '' }}
                        </td>
                    </tr>

                </tbody>
            </table>

      </div>
    </div>
  </div>
</div>
@endsection
