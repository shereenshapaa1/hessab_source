...
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Rating') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('sliders.create'))
                            <a class="btn btn-success pull-right text-white" href="{{ route('admin.rating.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                            ...
                            <table class="table table-sm table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>@lang('admin.Position_Rating')</th>
                                        <th>@lang('admin.commit')</th>
                                            ...
                                            <td class="text-left">
                                                @if (can('sliders.edit'))
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.rating.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('sliders.delete'))
                                                    <form action="{{ route('admin.rating.destroy', $item->id) }}"
