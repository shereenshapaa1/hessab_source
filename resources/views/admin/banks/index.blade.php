...
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Banks') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('banks.create'))
                            <a class="btn btn-success pull-right text-white" href="{{ route('admin.banks.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                                            ...
                                            <td class="text-left">
                                                @if (can('banks.edit'))
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.banks.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('banks.delete'))
                                                    <form action="{{ route('admin.banks.destroy', $item->id) }}"
