...
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Goals') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('types.create'))
                            <a class="btn btn-success pull-right text-white" href="{{ route('admin.goalmachines.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                    <hr>
                    @include('admin.goalmachines.search')
                                            ...
                                            <td class="text-left">
                                                @if (can('ratesMachine.edit'))
                                                    ...
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.goalmachines.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('ratesMachine.delete'))
                                                    <form action="{{ route('admin.goalmachines.destroy', $item->id) }}"
