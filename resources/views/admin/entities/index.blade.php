...
@section('tab_name', __('admin.Entities'))
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Entities') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('entities.create'))
                            <a class="btn btn-success pull-right text-white" href="{{ route('admin.entities.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                    <hr>
                    @include('admin.entities.search')
                                            ...
                                            <td class="text-left">
                                                @if (can('entities.edit'))
                                                    ...
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.entities.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('entities.destroy'))
                                                    <form action="{{ route('admin.entities.destroy', $item->id) }}"
