...
@section('tab_name', __('admin.Objectives'))
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('admin.Objectives') <span
                            class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                        @if (can('objectives.create'))

                            <a class="btn btn-success pull-right text-white"
                                href="{{ route('admin.objectives.create') }}">
                                <i class=" icon-plus"></i>
                            </a>
                        @endif
                    </h4>
                    <hr>
                    @include('admin.objectives.search')
                                ...
                                <tbody>
                                    @foreach ($result['items'] as $item)
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                @if (can('objectives.edit'))

                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('admin.objectives.edit', $item->id) }}">
                                                        <i class="text-white icon-note"></i>
                                                    </a>
                                                @endif
                                                @if (can('objectives.destroy'))
                                                    <form action="{{ route('admin.objectives.destroy', $item->id) }}"
